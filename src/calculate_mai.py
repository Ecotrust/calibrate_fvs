...
def calc_mai(si, sisp, sibase, vol_loc_grp, stockability):
    ...
    """
    si = np.asarray(si)
    sisp = np.asarray(sisp)
    sibase = np.asarray(sibase)
    vol_loc_grp = np.asarray(vol_loc_grp)
    stockability = np.asarray(stockability)
    mai = np.zeros_like(si)
    stk = np.where(np.isnan(stockability), 1.0, stockability)

    mask = (
        np.isin(sisp, [202, 17])
        & np.isin(vol_loc_grp, ["S26LWOR", "S26LWWA", "S26LCA"])
        & (sibase == 50)
    )
    ...
    mask = (
        np.isin(sisp, [242])
        & (np.isin(vol_loc_grp, ["S26LWWA", "S26LWACF", "S26LWOR", "S26LCA"]))
        & (sibase == 50)
    )
    ...
    mask = (vol_loc_grp == "S26LCAMIX") & (sibase == 50)
    mai[mask] = eq18(si[mask]) * stk[mask]

    mask = (
        np.isin(sisp, [20, 21, 15])
        & (np.isin(vol_loc_grp, ["S26LCA"]))
        & (sibase == 50)
    )
